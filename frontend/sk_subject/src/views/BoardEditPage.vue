<template>
  ...
  export default {
    ...
    methods: {
      async loadPost() {
        try {
          const response = await axiosInstance.get(`/board/${this.id}`);
          const { title, content, attachments } = response.data;
  
          this.title = title;
          this.content = content;
          if (attachments.length > 0) {
            this.attachmentUrl = attachments[0].url;
            this.attachmentType = attachments[0].type.startsWith("image") ? "image" : "file";
          }
        } catch (error) {
      ...
      async updatePost() {
        ...
        try {
          await axiosInstance.put(`/board/${this.id}`, {
            title: this.title,
